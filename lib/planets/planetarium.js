...
sketch(function() {
  ...
  this.setup = function() {
    ...
    _this.planets = [];
    for (i = 0; i <= 21; i++) {
      _this.planets.push(_this.createNewRandomPlanet(_this.planets, i));
    }
    return _this.setupMouseWheel();
  };
  this.createNewRandomPlanet = function(planets, index) {
    ...
    for (_i = 0, _len = planets.length; _i < _len; _i++) {
      ...
      if (this.dist(x, y, p.x, p.y) < (halfsize + (p.size / 2) + minDistBetweenPlanets)) {
        return this.createNewRandomPlanet(planets, index);
      }
    }
    rotationSpeed = this.PI / (500 + this.random(1500));
    name = "Planet #" + index;
    return new Planet(this, x, y, size, rotationSpeed, name);
  };
